...
<html class="theme-next pisces use-motion" lang="zh-Hans">
	...
	List<List> typeCountList = (List<List>)request.getAttribute("typeCountList");
	%>
      ...
      <div class="post-body">
          <div class="category-all-page">
            <div class="category-all-title">
                目前共计 <%=typeList.size() %> 个分类
            </div>
            <div class="category-all">
              <ul class="category-list">
              <%for(int i=0;i<typeList.size();i++){%>
              <%List list = (List)typeList.get(i);%>
              <li class="category-list-item"><a class="category-list-link" href="<%=basePath %>/articleListByType?type=<%=list.get(0) %>"><%=StringUtils.getTypeName(list.get(0)+"") %></a><span class="category-list-count"><%=list.get(1) %></span></li>
			<%} %>
              </ul>
            </div>
          </div>
